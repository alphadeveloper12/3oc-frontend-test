      ...
      <Accordion.Body className="px-4 py-3 transition-all duration-700 ease-in-out">
        <div className="text-xs space-y-1">
          ...
          {position !== "Participant" && (
            <>
              <h3 className="font-semibold text-xs">{txt.notificationTime}</h3>
              {notificationTime}
            </>
          )}
          <h3 className="font-semibold text-xs">{txt.location}</h3>
          <p>{props?.irl?.place}</p>
        </div>
          ...
          <div className="space-y-2 mt-2">
            <Row className="gap-2">
              ...
              <Col>
                <Button
                  onClick={_cancelIRL}
                  className="bg-gray-900 text-white py-1 px-3 rounded text-xs"
                >
        ...
        {position === positions[1] && (
          <div className="mt-2">
            ...
            >
              ...
              {txt.withdrawParticipant}
            </Button>
          </div>
        )}
        <div className="space-y-2 mt-3">
          <h3 className="font-semibold text-xs">{txt.organizer}</h3>
          {props?.irl?.organizer && (
            <UserProfileBadge connection={props?.irl?.organizer} />
          )}
          {props?.irl?.participants.length > 0 && (
            <h3 className="font-semibold text-xs">{txt.participants}</h3>
          )}
          ...
          {position !== "Participant" && (
            <>
              {props?.irl?.contacts.length > 0 && (
                <h3 className="font-semibold text-xs">{txt.backupBud}</h3>
              )}
          ...
          {props?.irl?.notes &&
            (position === "Backup Bud" || position === "Organizer") && (
              <>
                <h3 className="font-semibold text-xs">{txt.backupBudNotes}</h3>
