          ...
          <div className="my-4 w-full flex flex-col items-center">
            ...
            {edit ? (
              <>
                ...
                >
                  {form_information.map((field) => (
                    <div key={field.label} className="mt-3 mb-2">
                      ...
                      <input
                        ...
                        className="w-full h-10 px-4 mt-2 text-base text-black rounded-lg border focus:outline-none focus:border-[#26725e] transition-colors duration-300 opacity-70"
                        {...register(field.variable)}
                      />
                    </div>
                  ))}
                  <label className="block text-base font-medium text-white mt-3">
                    {txt.socials}
                  </label>
                  <div className="flex flex-wrap">
                    {socials_mapping.map((item) => (
                      <div key={item.title} className="flex my-2 w-full">
                        ...
                        <input
                          placeholder={"Enter your " + item.title + " link here"}
                  ...
                  <button
                    type="submit"
                    className="w-full py-2 bg-[#1f6254] text-white rounded-lg hover:bg-opacity-90 transition duration-300 mb-3 mt-3"
                  >
                    {txt.update}
                  </button>
                </form>
              ...
              <div className="flex flex-col items-center mt-2">
                ...
                <button
                  onClick={editProfile}
                  className="w-full py-2 bg-[#1f6254] text-white rounded-lg hover:bg-opacity-90 transition duration-300 my-2 mt-3"
                >
                  {txt.editProfile}
                </button>
                <button
                  onClick={logout}
                  className="w-full py-2 bg-[#9e9d9d] text-white rounded-lg hover:bg-opacity-90 transition duration-300 my-2"
                >
                  {txt.logout}
                </button>
